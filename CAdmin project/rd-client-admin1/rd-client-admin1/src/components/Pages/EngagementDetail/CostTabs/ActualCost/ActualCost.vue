<template>
  <div v-if="costList.length != 0 && !loader" class="table-responsive englistdata">
    <VTable class="w-full min-w-full divide-y divide-gray-200 bg-white costatable" :data="costList" :hide-sort-icons="true">
      <template #head>
        <tr>
        ...
        <tr v-for="row in rows" :key="row.id">
          ...
          <td>
            ...
            "
              ...
              <p
                v-if="row.actual_cost && row.actual_cost != 0"
                class="
              p-0
              rounded-radius6
              cursor-pointer
...
<script lang="ts" src="./ActualCost.ts"></script>
